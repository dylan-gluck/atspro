...
export const getJobs = query(listJobsSchema, async (params = {}) => {
	...
	const { status, limit = 20, offset = 0, sort = 'createdAt', order = 'desc' } = params;
...
>
	<input type="file" name="document" accept=".pdf,.md,.txt" required />
	<button type="submit"> Extract Resume </button>
</form>
...
{:then data}
	<div class="job-grid">
		{#each data.jobs as job}
			<div class="job-card">
				...
				<select value={job.status} onchange={(e) => handleStatusUpdate(job.id, e.target.value)}>
					<option value="tracked">Tracked</option>
					<option value="applied">Applied</option>
					<!-- ... -->
				</select>
	...
	{#if data.pagination.hasMore}
		<button onclick={() => jobs.refresh()}> Load More </button>
	{/if}
{/await}
```
...
{:then [jobData, resumeData]}
	...
	<div class="optimization-panel">
		...
		<button onclick={handleOptimize}> Generate Optimized Resume </button>
