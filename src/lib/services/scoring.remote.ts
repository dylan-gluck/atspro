...
function extractKeywords(text: string): string[] {
	...
	const techKeywords = [
		...
		'aws', 'docker', 'kubernetes', 'git', 'agile', 'scrum', 'sql', 'nosql',
	...
	const businessKeywords = [
		'leadership', 'management', 'strategy', 'budget', 'roi', 'stakeholder',
		'project management', 'cross-functional', 'collaboration', 'communication',
		'problem-solving', 'analytical', 'team', 'deadline', 'kpi', 'metrics'
	];
...
function findActionVerbs(content: string): string[] {
	const actionVerbs = [
		...
		'designed', 'developed', 'directed', 'established', 'executed', 'implemented',
		'improved', 'increased', 'initiated', 'led', 'managed', 'organized',
		'performed', 'planned', 'produced', 'resolved', 'streamlined', 'supervised'
	];
...
function generateRecommendations(analysis: any, jobDescription: string): string[] {
	...
	if (analysis.achievements < 3) {
		recommendations.push('Add more quantifiable achievements with specific numbers and percentages');
	}
	
	// Action verb recommendations
	if (analysis.actionVerbs < 5) {
		recommendations.push('Start bullet points with strong action verbs like "Led", "Developed", "Achieved"');
	}
	...
	if (!analysis.sections.hasSkills) {
		recommendations.push('Include a dedicated skills section with relevant technical and soft skills');
	}
