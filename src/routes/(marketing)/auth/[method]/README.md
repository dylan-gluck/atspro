...
let formConfig = $derived(() => {
  switch (method) {
    case 'sign-in': return { /* sign-in config */ };
    case 'sign-up': return { /* sign-up config */ };
    case 'forgot-password': return { /* reset config */ };
...
```svelte
{#if error}
  <div class="border-destructive/20 bg-destructive/5 text-destructive rounded-md border px-3 py-2 text-sm">
    {error}
  </div>
{/if}
```
