...
const recursiveWatcher = watch('./src', { recursive: true }, (eventType, filename) => {
...
class FileManager {
	...
	static async compareFiles(file1Path, file2Path) {
		...
		if (file1.size < 1024 * 1024) {
			// 1MB
			const [content1, content2] = await Promise.all([file1.bytes(), file2.bytes()]);
...
const results = await processFiles(['file1.txt', 'file2.txt', 'file3.txt'], async (file) => {
	const content = await file.text();
	...
	try {
		while (true) {
			...
			if (bytesProcessed % (1024 * 1024) === 0) {
				...
				console.log(
					`Processed ${(bytesProcessed / 1024 / 1024).toFixed(2)} MB at ${rate.toFixed(2)} MB/s`
				);
			}
		}
	} finally {
