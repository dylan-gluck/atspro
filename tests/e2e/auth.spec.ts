...
test.describe('Authentication Flow', () => {
	...
	test('should handle registration errors', async ({ page }) => {
		...
		const hasEmailValidation = await emailField.evaluate((el: HTMLInputElement) => !el.validity.valid);
		const hasPasswordValidation = await passwordField.evaluate((el: HTMLInputElement) => !el.validity.valid);
